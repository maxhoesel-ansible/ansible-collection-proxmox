...
options:
  ...
  validate_certs:
    description:
      - Validate SSL certificate of the PVE host upon connectiong
    default: false
  vmid:
    description:
...
def main():
    ...
    if not HAS_PROXMOXER:
        m.fail_json(msg="This module requires proxmoxer. Please install it with pip")
    ...
    try:
        ...
        m.fail_json(msg="Could not connect to PVE cluster. Exception: {0}".format(e))

    if PVE_MAJOR_VERSION <= 3:
        m.fail_json(msg="This module only supports the new HA stack introduced in Proxmox PVE 4.0")
    ...
    if not vmid:
        try:
            ...
            m.fail_json(msg="Could not VMID for name {0} in cluster resources".format(m.params["name"]))
    ...
    if _resources:
        ...
        try:
            _resources[0].pop("type") # No need for this key as we don't expose it
        except KeyError:
            pass
    ...
    if m.params["state"] == "absent":
        if not current_config:
            m.exit_json(changed=False, msg="Resource {0} does not exist".format(vmid))
        else:
            # Remove the resource
            if not m.check_mode:
                try:
                    ...
                    m.fail_json(msg="Could not remove HA resource {0}. Exception: {1}".format(m.params["vmid]"], e))
        ...
        for key in list(desired_config): # list() creates a copy of the dict keys so that we can iterate
        ...
        if not current_config:
            ...
            if not m.check_mode:
                try:
                    ...
                    m.fail_json(msg="Could not add HA resource {0}. Exception: {1}".format(vmid, e))
            m.exit_json(changed=True, msg="Added resource {0}".format(vmid), resource=desired_config)
        else:
            ...
            if desired_config == current_config:
                ...
                if not m.check_mode:
                    ...
                    try:
                        ...
                        m.fail_json(msg="Could not change HA resource {0}. Exception: {1}".format(vmid, e))
